...
class train_scrape:
	...
	def get_path_as_a_list(self):
		...
		for row in self.response.xpath('//tbody/tr'):
			col=row.xpath('.//td/text()').extract()
			l.append(str(re.search(r'(.*)[(]',col[0]).group(1)))
		return l
...
class pnr_scrape:
	def __init__(self,num):
		...
		#open and read the page/fetch html contents
		res=requests.post('http://www.getpnrstatus.co.in?pnrno='+str(self.pnr))
		the_page=res.text		
...
pnr=pnr_scrape(pnr_number)#this one is a valid pnr number
...
train_path = trn.get_path_as_a_list() #stn path
